#!/usr/bin/env python3
"""
Simple statistical analysis of swing pivot bounce zones strategies.
"""
...
if top_performers['avg_loss'].mean() != 0:
...
if bottom_performers['avg_loss'].mean() != 0:
...
print(correlations['total_return'].sort_values(ascending=False).round(3))
...
with open('swing_pivot_detailed_stats.txt', 'w') as f:
    ...
    f.write("\n\nCorrelation Matrix:\n")
    f.write(str(correlations.round(3)))

# Export top performers for further analysis
top_10_pct = df.nlargest(150, 'total_return')  # Top 10% of 1500 strategies
top_10_pct.to_csv('swing_pivot_top_performers.csv', index=False)
print(f"\nExported top 150 performers to swing_pivot_top_performers.csv")
