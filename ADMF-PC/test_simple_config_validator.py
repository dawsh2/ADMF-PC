...
from src.core.config import ConfigSchemaValidator, SchemaValidationError
...
def test_normalization():
    ...
    if result.normalized_config:
        ...
        print(f"    - type: {result.normalized_config.get('type')}")
...
def test_complex_validation():
    ...
    complex_config = {
        "name": "Complex Backtest",
        "description": "Multi-strategy backtest with risk management",
        "data": {
        ...
        "risk": {
            "limits": [
                {"type": "position", "max_position": 100000},
                {"type": "exposure", "max_exposure_pct": 90},
                {"type": "drawdown", "max_drawdown_pct": 15},
                {"type": "daily_loss", "max_daily_loss_pct": 2}
            ],
            "position_sizers": [
                {
                    "name": "vol_sizer",
    ...
    result = validator.validate(complex_config, "backtest")
    print(f"\nComplex configuration validation:")
    print(f"  Is valid: {result.is_valid}")
    print(f"  Errors: {result.errors}")
    print(f"  Warnings: {result.warnings}")
    print(f"  Strategies: {len(complex_config['strategies'])}")
    print(f"  Risk limits: {len(complex_config['risk']['limits'])}")


if __name__ == "__main__":
    print("=" * 60)
    print("Simple Configuration Validator Tests")
    print("=" * 60)
