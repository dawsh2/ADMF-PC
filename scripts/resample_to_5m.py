#!/usr/bin/env python3
"""
Resample 1-minute SPY data to 5-minute bars.

This script:
1. Reads SPY_1m.parquet
2. Resamples to 5-minute OHLCV bars
3. Saves as SPY_5m.parquet and SPY_5m.csv
"""
...
def resample_to_5m(df_1m):
    ...
    df_5m = df_1m.resample('5min').agg(agg_rules)
    ...
    df_5m = df_5m.rename(columns={'index': 'timestamp'})
...
def main():
    ...
    output_parquet = data_dir / 'SPY_5m.parquet'
    output_csv = data_dir / 'SPY_5m.csv'
    ...
    print(f"Date range: {df_5m['timestamp'].min()} to {df_5m['timestamp'].max()}")
    
    # Show sample of the data
    print("\nSample of 5-minute data:")
    print(df_5m.head())
    
    # Save the resampled data
    print(f"\nSaving to {output_parquet}...")
    df_5m.to_parquet(output_parquet, index=False)
    ...
    # Compare to 15m data
    df_15m = pd.read_parquet(data_dir / 'SPY_15m.parquet')
    print(f"\nComparison:")
    print(f"  1m data: {len(df_1m)} bars")
    print(f"  5m data: {len(df_5m)} bars (ratio: {len(df_1m)/len(df_5m):.1f}:1)")
    print(f"  15m data: {len(df_15m)} bars (ratio: {len(df_5m)/len(df_15m):.1f}:1)")
