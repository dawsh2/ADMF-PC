#!/usr/bin/env python3
"""
Analyze MA crossover momentum strategy by identifying complete trade cycles.
...
def analyze_ma_crossover_trades(workspace_path, data_path):
    ...
    print('ANALYZING MA CROSSOVER MOMENTUM COMPLETE TRADE CYCLES')
    print('=' * 60)
    ...
    print(f'\\nHolding period distribution (top 10):')
    ...
    # Use actual trade frequency from data
    total_data_bars = returns_df['exit_idx'].max() - returns_df['entry_idx'].min()
    data_days = total_data_bars / bars_per_day
    actual_trades_per_day = total_trades / data_days
    actual_trades_per_year = actual_trades_per_day * trading_days_per_year
    
    annual_return = avg_return_per_trade * actual_trades_per_year
    annual_volatility = volatility_per_trade * (actual_trades_per_year ** 0.5)
    ...
    print(f'  Actual trades per day: {actual_trades_per_day:.1f}')
    print(f'  Actual trades per year: {actual_trades_per_year:.0f}')
    ...
    for cost_bps in cost_levels:
        ...
        net_return_per_trade = avg_return_per_trade - (2 * cost_pct)  # Round trip
        net_annual_return = net_return_per_trade * actual_trades_per_year
...
if __name__ == "__main__":
    ...
    results = analyze_ma_crossover_trades(workspace_path, data_path)
    
    if results:
        print(f'\\n🎯 MA CROSSOVER MOMENTUM ANALYSIS:')
