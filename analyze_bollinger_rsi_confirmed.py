#!/usr/bin/env python3
"""
Analyze Bollinger RSI Confirmed strategy performance
"""
...
# Load signals
signal_file = Path("/Users/daws/ADMF-PC/workspaces/signal_generation_2434c159/traces/SPY_1m/signals/bollinger_rsi_confirmed/SPY_compiled_strategy_0.parquet")
signals = pd.read_parquet(signal_file)

print("="*60)
print("BOLLINGER RSI CONFIRMED STRATEGY ANALYSIS")
print("="*60)
...
if len(trades_df) > 0:
    ...
    for pct in [5, 10, 25, 50, 75, 90, 95]:
        val = np.percentile(trades_df['pnl_pct'], pct)
        print(f"  {pct}th: {val:.3f}%")
    
    # Monthly estimate
    total_bars = trades_df['exit_idx'].max() - trades_df['entry_idx'].min()
    ...
    print("\n" + "="*60)
    print("COMPARISON TO BACKTESTED RSI DIVERGENCE:")
