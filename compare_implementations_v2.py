...
def main():
    ...
    sweep_path = Path("config/bollinger/results/20250623_062931/traces/bollinger_bands/SPY_5m_compiled_strategy_40.parquet")
    ...
    if sweep_path.exists():
        ...
        print(f"\nSweep signals shape: {sweep_signals.shape}")
        print(f"Sweep signals columns: {sweep_signals.columns.tolist()}")
    ...
    if ensemble_path.exists():
        ...
        print(f"\nEnsemble signals shape: {ensemble_signals.shape}")
        print(f"Ensemble signals columns: {ensemble_signals.columns.tolist()}")
        print(f"First 5 rows:\n{ensemble_signals.head()}")
