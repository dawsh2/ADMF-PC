#!/usr/bin/env python3
"""Analyze Swing Pivot Bounce Zones 1m with fixed sustained signals."""
...
def extract_parameters(strategy_idx):
    ...
    # Parameter order from zones 1m config:
    # sr_period: [10, 15, 20, 30, 40, 50] = 6 values
    # min_touches: [2, 3, 4] = 3 values  
    # entry_zone: [0.001, 0.0015, 0.002, 0.0025, 0.003] = 5 values
    # exit_zone: [0.001, 0.0015, 0.002, 0.0025, 0.003] = 5 values
    # min_range: [0.002, 0.003, 0.004, 0.005] = 4 values
    # Total: 6 * 3 * 5 * 5 * 4 = 1800 combinations
...
def main():
    ...
    print(f"Analyzing 1m zones optimization with FIXED sustained signals")
    print("Strategy: swing_pivot_bounce_zones on 1-minute data")
    print("Total combinations: 1800\n")
    ...
    # Overall statistics
    print(f"\n=== OVERALL STATISTICS ===")
    print(f"Average trades per strategy: {results_df['trades'].mean():.0f}")
    print(f"Max trades: {results_df['trades'].max()}")
    print(f"Average holding period: {results_df['avg_bars_held'].mean():.1f} bars")
    print(f"Average win rate: {results_df['win_rate'].mean()*100:.1f}%")
    ...
    # High frequency analysis (>1000 trades)
    high_freq = results_df[results_df['trades'] > 1000].sort_values('annual_return', ascending=False)
    if len(high_freq) > 0:
        print(f"\n=== HIGH FREQUENCY STRATEGIES (>1000 trades) ===")
        print(f"Count: {len(high_freq)}")
        print(f"Best annual return: {high_freq['annual_return'].max()*100:.2f}%")
        print(f"Average bps/trade: {high_freq['avg_return_bps'].mean():.2f}")
        print("\nTop 5 high-frequency:")
        for _, row in high_freq.head(5).iterrows():
            print(f"Entry:{row['entry_zone']:.3f} Exit:{row['exit_zone']:.3f} → "
                  f"{row['trades']:.0f} trades, {row['avg_return_bps']:.1f} bps, "
                  f"{row['annual_return']*100:.1f}% annual")
    
    # Compare with original results
    print("\n=== COMPARISON WITH ORIGINAL ===")
    print("Original swing pivot bounce: ~3600 trades, 0.04 bps/trade raw")
    print("With high volatility filter: 148 trades, 1.26 bps/trade")
    avg_trades = results_df['trades'].mean()
    print(f"\nCurrent average: {avg_trades:.0f} trades")
    if avg_trades > 2000:
        print("✓ Good trade count for filtering!")
    else:
        print("✗ May need wider zones for more trades")
    
    # Save results
    output_file = 'swing_zones_1m_fixed_results.csv'
