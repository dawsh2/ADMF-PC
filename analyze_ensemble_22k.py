...
def load_sparse_signals(signal_path: str, start_time: pd.Timestamp, end_time: pd.Timestamp) -> pd.DataFrame:
    """Load sparse signals and filter to time range."""
...
def reconstruct_signal_timeline(sparse_signals: pd.DataFrame, price_index: pd.DatetimeIndex) -> pd.Series:
    """Reconstruct full signal timeline from sparse storage."""
    # Create full timeline
    signal_timeline = pd.Series(0, index=price_index, dtype=int)
    
    # Forward fill sparse signals
    if len(sparse_signals) > 0:
        # Align sparse signals to price index
        sparse_aligned = sparse_signals.reindex(price_index, method='ffill')
        signal_timeline = sparse_aligned['signal'].fillna(0).astype(int)
    
    return signal_timeline

def calculate_trades_from_signals(signals: pd.Series, prices: pd.Series) -> pd.DataFrame:
...
def calculate_performance_metrics(trades_df: pd.DataFrame, total_bars: int) -> Dict:
    ...
    return {
        ...
        'avg_trade_duration': trades_df['duration'].mean() if len(trades_df) > 0 else pd.Timedelta(0)
    }
...
def main():
    ...
    start_time = spy_data.index[0]
    end_time = spy_data.index[-1]
    ...
    for ensemble_type in ensemble_types:
        ...
        sparse_signals = load_sparse_signals(signal_path, start_time, end_time)
        
        # Reconstruct signal timeline
        signal_timeline = reconstruct_signal_timeline(sparse_signals, spy_data.index)
        
        # Calculate trades
        trades_df = calculate_trades_from_signals(signal_timeline, spy_data['close'])
    ...
    sparse_regimes = load_sparse_signals(classifier_path, start_time, end_time)
    regime_timeline = reconstruct_signal_timeline(sparse_regimes, spy_data.index)
    ...
    for ensemble_type in ensemble_types:
        ...
        if len(trades_df) > 0:
            ...
            trades_df['entry_regime'] = trades_df['entry_time'].apply(
                lambda x: int(regime_timeline.asof(x)) if x in regime_timeline.index else -1
            )
