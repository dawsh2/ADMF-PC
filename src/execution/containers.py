"""
Execution-specific container implementations using event-driven architecture.

This version implements proper event-driven communication for sharing portfolio
state between PortfolioContainer and RiskContainer.
"""
...
from ..execution.protocols import OrderSide, OrderType, Order
    ...
    async def _process_signals(self, timestamp) -> None:
        ...
        if self.strategy and self.last_indicators:
            ...
            if signals:
                ...
                logger.warning("⚠️ No signals generated by strategy")
        else:
...
class RiskContainer(BaseComposableContainer):
    ...
    def __init__(self, config: Dict[str, Any], container_id: str = None):
        ...
        self._cached_portfolio_state = None  # Cache portfolio state from events
    ...
    async def process_event(self, event: Event) -> Optional[Event]:
        """Process events from internal event bus."""
        # Call receive_event which handles async task creation
    ...
    async def _initialize_self(self) -> None:
        ...
        # Subscribe to PORTFOLIO events to receive portfolio state updates
        self.event_bus.subscribe(EventType.PORTFOLIO, self._handle_portfolio_event_sync)
    ...
    def _check_risk_limits(self, signal) -> bool:
        ...
        # Get current portfolio state from cache
    ...
    def _get_portfolio_state(self):
        ...
        # If no cached state yet, create a temporary one
    ...
    def _handle_portfolio_event_sync(self, event: Event) -> None:
        """Handle portfolio update event synchronously (for event bus subscription)."""
        asyncio.create_task(self._handle_portfolio_event(event))
    
    async def _handle_portfolio_event(self, event: Event) -> None:
        """Handle portfolio state update events."""
        logger.debug(f"🔍 RiskContainer received PORTFOLIO event with update_type: {event.payload.get('update_type')}")
        if event.payload.get('update_type') == 'state':
            self._cached_portfolio_state = event.payload.get('portfolio_state')
            if self._cached_portfolio_state:
                positions = self._cached_portfolio_state.get_all_positions()
                logger.info(f"📊 RiskContainer cached portfolio state update - Cash: ${self._cached_portfolio_state.get_cash_balance():.2f}, Positions: {len(positions)}")
                if positions:
                    for symbol, pos in positions.items():
                        logger.debug(f"🔍 Cached position: {symbol} - quantity: {getattr(pos, 'quantity', 'N/A')}")
            else:
                logger.warning("⚠️ RiskContainer received PORTFOLIO event but portfolio_state is None")
    ...
    def _create_order(self, symbol: str, side: OrderSide, quantity: float) -> Order:
    ...
    async def _handle_system_event(self, event: Event) -> None:
        """Handle system events like END_OF_DATA."""
        logger.info(f"🔧 RiskContainer _handle_system_event called with message: {event.payload.get('message')}")
        message = event.payload.get('message')
        if message == 'END_OF_DATA':
            logger.info("🏁 RiskContainer received END_OF_DATA event")
            
            # Debug logging for cached portfolio state
            logger.info(f"🔍 RiskContainer cached portfolio state exists: {self._cached_portfolio_state is not None}")
            if self._cached_portfolio_state:
                logger.info(f"🔍 Cached portfolio state type: {type(self._cached_portfolio_state)}")
                logger.info(f"🔍 Portfolio state attributes available: {hasattr(self._cached_portfolio_state, 'get_all_positions')}")
            
            # Get current positions from cached portfolio state
            if self._cached_portfolio_state:
                positions = self._cached_portfolio_state.get_all_positions()
                logger.info(f"🔍 Positions from portfolio state: {positions}")
                logger.info(f"🔍 Positions type: {type(positions)}, Length: {len(positions) if positions else 0}")
                
                if positions:
                    logger.info(f"📊 RiskContainer closing {len(positions)} positions at end of data")
                    
                    # Generate close orders for all positions
                    close_orders = []
                    
                    for symbol, position in positions.items():
                        logger.debug(f"🔍 Processing position for {symbol}: {position}")
                        logger.debug(f"🔍 Position type: {type(position)}")
                        logger.debug(f"🔍 Position attributes: {dir(position)}")
                        
                        if hasattr(position, 'quantity'):
                            logger.debug(f"🔍 Position quantity: {position.quantity}, type: {type(position.quantity)}")
                            if position.quantity != 0:
                                ...
                                logger.debug(f"🔍 Creating close order: {symbol} {close_side} {close_quantity}")
                                
                                # Create close order directly
                                ...
                                if close_order:
                                    ...
                                    logger.info(f"📋 Created close order for {symbol}: {close_side} {close_quantity} shares")
                                else:
                                    logger.warning(f"⚠️ Failed to create close order for {symbol}")
                            else:
                                logger.debug(f"🔍 Skipping {symbol} - zero quantity")
                        else:
                            logger.warning(f"⚠️ Position for {symbol} has no quantity attribute")
                    
                    # Emit close orders if any
                    if close_orders:
                        logger.info(f"📋 Generated {len(close_orders)} close orders")
                        ...
                        order_event = Event(
                            ...
                            payload={
                                'orders': close_orders,
                                'source': self.metadata.container_id,
                                'reason': 'end_of_data_position_close'
                            },
                            timestamp=event.timestamp
                        )
                        ...
                        logger.info("📊 No orders generated for position closure")
                else:
                    logger.info("📊 No open positions to close at end of data")
            else:
                logger.info("📊 No portfolio state available - no positions to close")
...
class PortfolioContainer(BaseComposableContainer):
    ...
    def on_output_event(self, handler):
        """Register handler for output events (used by pipeline adapter)."""
        # Portfolio forwards signals to next stage
        self.event_bus.subscribe(EventType.SIGNAL, handler)
        # Also publish portfolio state updates
        self.event_bus.subscribe(EventType.PORTFOLIO, handler)
        
    @property
    def expected_input_type(self):
        """PortfolioContainer expects SIGNAL events."""
        return EventType.SIGNAL
        
    def receive_event(self, event: Event):
        """Receive and process events from pipeline adapter."""
        if event.event_type == EventType.SIGNAL:
            # Forward signals through the pipeline
            logger.info(f"📊 PortfolioContainer received SIGNAL event with {len(event.payload.get('signals', []))} signals")
            logger.info(f"📤 PortfolioContainer forwarding signals to next stage")
    ...
    async def _initialize_self(self) -> None:
        ...
        # Convert to Decimal for PortfolioState
        self.portfolio_state = PortfolioState(initial_capital=Decimal(str(initial_capital)))
        ...
        try:
            ...
            if hasattr(fill, 'side'):
                ...
                if hasattr(side_value, 'value'):
                    side = str(side_value.value).upper()
                # Handle integer side values (1=BUY, 2=SELL)
                ...
                logger.debug(f"🔍 Converted fill.side {side_value} (type: {type(side_value)}) to '{side}'")
            else:
                side = 'BUY'  # Default
            ...
            # Update cash balance for trade (convert to Decimal for consistent arithmetic)
            trade_value = Decimal(str(fill.quantity)) * Decimal(str(fill.price))
            commission = Decimal(str(getattr(fill, 'commission', 0)))
            
            if side == 'BUY':
                # Buying - reduce cash by trade value + commission
                cash_change = -(trade_value + commission)
            else:
                # Selling - increase cash by trade value - commission
                cash_change = trade_value - commission
            
            # Apply cash change
            self.portfolio_state._cash_balance += cash_change
            
            logger.info(f"💼 PortfolioContainer updated: {fill.symbol} {side} {fill.quantity} @ {fill.price}")
            logger.info(f"   📈 Position: {position.quantity if position else 'None'}")
            logger.info(f"   💰 Cash change: {cash_change:.2f}, New balance: {self.portfolio_state._cash_balance:.2f}")
            
            # Publish portfolio state update after fill
            await self._publish_portfolio_state()
    ...
    async def _publish_portfolio_state(self) -> None:
        """Publish current portfolio state as an event."""
        if self.portfolio_state:
            portfolio_event = Event(
                ...
                payload={
                    ...
                    'positions': {symbol: {
                        ...
                        'market_value': float(pos.quantity * pos.current_price) if hasattr(pos, 'current_price') else 0
            ...
            # Publish to own event bus for local handlers
            self.event_bus.publish(portfolio_event)
            
            # Also publish to siblings via parent
            if hasattr(self, 'publish_internal'):
                self.publish_internal(portfolio_event, scope="siblings")
            
            logger.info(f"📢 Published portfolio state update - Cash: ${self.portfolio_state.get_cash_balance():.2f}, Positions: {len(self.portfolio_state.get_all_positions())}")
...
def register_execution_containers():
    ...
    # Additional specialized containers can be registered here
    # engine.register_container_type("classifier", ClassifierContainer)
    # engine.register_container_type("analysis", AnalysisContainer)
