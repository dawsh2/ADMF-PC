...
class PortfolioContainer(BaseComposableContainer):
    ...
    async def _handle_fill_event(self, event: Event) -> None:
        ...
        try:
            ...
            # Apply cash change - ensure consistent types
            if isinstance(self.portfolio_state._cash_balance, Decimal):
                # Cash balance is Decimal, cash_change is Decimal - direct addition
                self.portfolio_state._cash_balance += cash_change
            else:
                # Cash balance is float, convert cash_change to float
                self.portfolio_state._cash_balance += float(cash_change)
        ...
        try:
            ...
            for symbol, position in positions.items():
                if hasattr(position, 'quantity') and position.quantity != 0:
                    ...
                    trade_value = original_quantity * current_price
                    
                    # Update cash balance (selling long positions adds cash, covering short positions costs cash)
                    # Ensure consistent Decimal arithmetic
                    cash_change = Decimal(str(trade_value))
...
class ExecutionContainer(BaseComposableContainer):
    ...
    async def _handle_system_event(self, event: Event) -> None:
        ...
        if message in ['END_OF_DATA', 'END_OF_DAY', 'FORCE_CLOSE']:
            ...
            await self._close_all_positions_directly(reason=message)
