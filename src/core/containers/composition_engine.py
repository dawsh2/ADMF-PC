...
This module implements the engine that coordinates container composition
based on patterns defined by the Coordinator.
"""
...
from .composable import (
    ComposableContainerProtocol, ContainerCompositionProtocol, 
    ContainerRegistryProtocol, ContainerRole, ContainerPattern,
    ContainerMetadata, ContainerLimits
)
...
class ContainerRegistry:
    ...
    def _load_default_patterns(self) -> None:
        ...
        full_backtest = ContainerPattern(
            name="full_backtest",
            description="Complete backtest with data → indicators → classifier → risk → portfolio → strategy → execution",
            structure={
                "root": {
                    "role": "data",
                    "children": {
        ...
        simple_backtest = ContainerPattern(
            ...
            required_capabilities={"data.historical", "indicator.computation", "risk.management", "execution.backtest"},
            default_config={
                "data": {"source": "historical"},
...
class ContainerCompositionEngine:
    """Engine for composing containers according to patterns."""
    ...
    def _infer_required_indicators(self, config: Dict[str, Any]) -> Set[str]:
        ...
        for strategy_config in all_strategies:
            ...
            # Infer indicators based on strategy type and parameters
            if strategy_type == 'momentum':
                ...
                required_indicators.add(f'RSI_{rsi_period}' if rsi_period != 14 else 'RSI')
                
            elif strategy_type == 'mean_reversion':
                # Mean reversion strategies typically need moving averages and Bollinger Bands
                required_indicators.update(['SMA_20', 'BB_20', 'RSI'])
                
            elif strategy_type == 'trend_following':
                # Trend following strategies typically need multiple timeframe MAs
                required_indicators.update(['SMA_10', 'SMA_20', 'SMA_50', 'MACD'])
                
            # Add explicit indicators if specified in config
            explicit_indicators = parameters.get('indicators', [])
            required_indicators.update(explicit_indicators)
    ...
    def compose_pattern(
        ...
        # Infer required indicators from strategy configurations
        required_indicators = self._infer_required_indicators(base_config)
        if required_indicators:
            logger.info(f"Inferred indicators from strategy config: {required_indicators}")
            # Inject indicators into indicator container config (use 'indicator' key to match ContainerRole.INDICATOR.value)
            base_config['indicator'] = {
    ...
    def _build_container_tree(
        ...
        parent_container: ComposableContainerProtocol = None
    ) -> ComposableContainerProtocol:
...
_global_composition_engine = ContainerCompositionEngine(_global_registry)
...
def get_global_composition_engine() -> ContainerCompositionEngine:
    """Get the global composition engine."""
    return _global_composition_engine
...
def compose_pattern(
    ...
    """Compose containers using global composition engine."""
    return _global_composition_engine.compose_pattern(pattern_name, config_overrides)