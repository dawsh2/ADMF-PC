...
This uses the bb_rsi_divergence feature in the feature hub to track
...
def bollinger_rsi_exact(features: Dict[str, Any], bar: Dict[str, Any], params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
    Exact implementation of the profitable Bollinger + RSI divergence pattern.
    
    This uses the bb_rsi_divergence feature which tracks:
    1. Price extremes outside Bollinger Bands
    2. RSI divergences (price makes new extreme, RSI doesn't)
    3. Confirmation when price returns inside bands
    ...
    # Get divergence tracking data
    divergence_data = features.get('bb_rsi_divergence', {})
    ...
    if middle_band and abs(price - middle_band) / middle_band <= exit_threshold:
        ...
        if divergence_data.get('confirmed_long', False):
            signal_value = 1  # Long entry
            entry_reason = f"Bullish divergence confirmed - Strength: {divergence_data.get('divergence_strength', 0):.1f}"
        elif divergence_data.get('confirmed_short', False):
            signal_value = -1  # Short entry
            entry_reason = f"Bearish divergence confirmed - Strength: {divergence_data.get('divergence_strength', 0):.1f}"
    ...
    return {
        ...
        'strategy_id': 'bollinger_rsi_exact',
        ...
        'metadata': {
            ...
            'has_bullish_divergence': divergence_data.get('has_bullish_divergence', False),
            'has_bearish_divergence': divergence_data.get('has_bearish_divergence', False),
            'confirmed_long': divergence_data.get('confirmed_long', False),
            'confirmed_short': divergence_data.get('confirmed_short', False),
            'bars_since_divergence': divergence_data.get('bars_since_divergence'),
