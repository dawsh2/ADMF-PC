...
class RiskLimits:
    """
    Aggregated risk limits container for easy configuration.
    
    Provides a simple interface for common risk management scenarios.
    """
    ...
    def __init__(self,
        ...
        """
        Initialize risk limits with common parameters.
        ...
        if max_position_size > 0:
            self.limits.append(MaxPositionLimit(
                name="max_position",
                max_position_value=Decimal(str(max_position_size))
            ))
        
        # Add exposure limit
        if max_sector_exposure > 0:
            self.limits.append(MaxExposureLimit(
                name="max_exposure",
                max_exposure_pct=Decimal(str(max_sector_exposure * 100))
            ))
        
        # Add drawdown limit
        if max_drawdown > 0:
            self.limits.append(MaxDrawdownLimit(
    ...
    def check_signal(self, signal) -> bool:
        ...
        Returns:
            True if signal passes all limits
        """
        # For now, return True - actual implementation would
        # check each limit with portfolio state
        return True
    
    def check_order(self, order: Order, portfolio_state: PortfolioStateProtocol) -> tuple[bool, Optional[str]]:
        """
        ...
        for limit in self.limits:
            is_allowed, reason = limit.check_order(order, portfolio_state)
            if not is_allowed:
                return False, reason
    ...
    def get_limit_info(self) -> Dict[str, Any]:
        """Get information about all limits."""
        return {
            "total_limits": len(self.limits),
            "limits": [limit.get_limit_info() for limit in self.limits]
        }