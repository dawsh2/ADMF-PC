...
# from ...core.events.storage.signals import SignalStorageManager  # TODO: Fix this import

# Temporary stub until events storage is fixed
class SignalStorageManager:
    def __init__(self, *args, **kwargs):
        pass
from ..protocols import SignalStreamer as SignalStreamerProtocol
...
class SignalStreamerComponent:
    ...
    def stream_signals(self) -> Iterator[Event]:
        ...
        for bar_idx in self.bar_indices[self.current_index:]:
            signals = self.signals_by_bar.get(bar_idx, [])
            for signal in signals:
                # Extract classifier states from signal data
                classifier_states = signal.get('classifier_states', {})
                yield self._create_signal_event(signal, bar_idx, classifier_states)
    ...
    def reset(self) -> None:
        ...
        # Reset boundary handlers
        for handler in self.boundary_handlers.values():
            handler.in_position = False
            handler.position_open_regime = None
            handler.position_open_bar = None
