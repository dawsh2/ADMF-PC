# Fixed execution trace analysis - uses correct timestamps from metadata
import pandas as pd
import numpy as np
...
if fills_files:
    ...
    # Extract actual execution timestamp from metadata
    actual_timestamps = []
    for _, row in fills_df.iterrows():
        meta = row['metadata']
        if isinstance(meta, dict):
            actual_timestamps.append(pd.to_datetime(meta.get('executed_at', row['ts'])))
        else:
            actual_timestamps.append(pd.to_datetime(row['ts']))
    
    fills_df['timestamp'] = actual_timestamps
    fills_df = fills_df.sort_values('timestamp').reset_index(drop=True)
    
    print(f"✅ Loaded {len(fills_df)} fills")
    print(f"Date range: {fills_df['timestamp'].min()} to {fills_df['timestamp'].max()}")
else:
...
if len(execution_trades) > 0:
    print(f"\n{'='*120}")
    print("EXECUTION ENGINE TRADE-BY-TRADE ANALYSIS (WITH CORRECT TIMESTAMPS)")
    print(f"{'='*120}")
    ...
    # Save to CSV
    csv_filename = 'execution_trades_bollinger_fixed.csv'
