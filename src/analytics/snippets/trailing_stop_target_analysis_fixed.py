...
# FIXED: Correctly identifies trailing stops vs regular stops
...
print("📊 Trailing Stop/Target Analysis for Bollinger Strategies (FIXED)")
print("=" * 80)
...
def extract_trades_with_trailing(signal_file, market_data, initial_stop_pct, trail_stop_pct, 
    ...
    for idx, row in df.iterrows():
        ...
        if current_pos == 0 and signal != 0:
            # Entry - we enter at the close of the bar with the signal
            current_pos = signal
            entry = {
                'time': row['ts'], 
                'price': row['px'], 
            ...
            if entry:
                # Find market data from entry time (inclusive) to current signal time
                # Important: >= entry time to include the entry bar itself
                ...
                if entry['direction'] > 0:  # Long
                    # Calculate initial stop/target levels
                    initial_stop_level = entry_price * (1 - initial_stop_pct/100) if initial_stop_pct > 0 else 0
                    initial_target_level = entry_price * (1 + initial_target_pct/100) if initial_target_pct > 0 else float('inf')
                    
                    stop_price = initial_stop_level
                    target_price = initial_target_level
                    ...
                    for bar_idx, bar in trade_bars.iterrows():
                        ...
                        if initial_stop_pct > 0 and current_low <= stop_price:
                            ...
                            # Fixed: Properly determine if stop has trailed
                            exit_type = 'trailing_stop' if stop_has_trailed else 'stop'
                    ...
                    # Calculate initial stop/target levels
                    initial_stop_level = entry_price * (1 + initial_stop_pct/100) if initial_stop_pct > 0 else float('inf')
                    initial_target_level = entry_price * (1 - initial_target_pct/100) if initial_target_pct > 0 else 0
                ...
                # Apply execution cost (round trip)
...
if results:
    ...
    # Save results
    output_path = RESULTS_DIR / 'trailing_stop_target_analysis_fixed.csv'
    ...
    print("\n4. BUG FIX APPLIED:")
    print("   - Fixed: Now correctly identifies trailing stops vs regular stops")
    print("   - Trailing stop = stop has moved from initial position")
    print("   - Regular stop = stop at initial position when hit")
