...
original_update_bar = FeatureHub.update_bar
...
def patched_execute_components_individually(self, symbol: str, features: Dict[str, Any], 
    ...
    # Batch strategy timing
    batch_size = 100
    batch_times = []
    
    for i, (component_id, component_info) in enumerate(ready_strategies):
        try:
            if i % batch_size == 0:
    ...
    if batch_times:
        timing_data['strategy_batch_100'].extend(batch_times)
...
def patched_update_features(self, symbol: str) -> None:
    ...
    for feature_type, feature_list in features_by_type.items():
        ...
        type_time = time.perf_counter() - type_start
        timing_data[f'feature_type_{feature_type}_batch'].append(type_time)
...
def patched_update_bar(self, symbol: str, bar: Dict[str, float]) -> None:
    """Patched version with timing."""
    start = time.perf_counter()
    original_update_bar(self, symbol, bar)
    duration = time.perf_counter() - start
    timing_data['feature_hub_update_bar'].append(duration)
...
FeatureHub.update_bar = patched_update_bar

# Fix the FEATURE_REGISTRY reference
import src.strategy.components.features.hub
FeatureHub.FEATURE_REGISTRY = src.strategy.components.features.hub.FEATURE_REGISTRY
    ...
    for section, times in sorted(timing_data.items()):
        if times:
            ...
            if len(times) > 1:
                ...
                # Add percentiles for strategies
...
def run_profiling_test():
    ...
    print(f"Setting up profiling test...")
    print(f"Config has {len(config.get('strategies', []))} strategy configs")
    
    # Create topology directly
    from src.core.coordinator.topology import create_topology
    ...
    topology = create_topology('backtest', config)
    
    print(f"Created topology with {len(topology['containers'])} containers")
    print(f"Stateless components: {list(topology['stateless_components'].keys())}")
    
    # Count strategies
    strategy_count = 0
    if 'strategies' in topology['stateless_components']:
        strategy_count = len(topology['stateless_components']['strategies'])
    print(f"Total strategies created: {strategy_count}")
    
    # Get the workflow manager and run
    from src.core.coordinator.workflows.workflow_manager import WorkflowManager
    
    print("\nRunning backtest with timing measurements...")
    start_time = time.time()
    
    workflow_manager = WorkflowManager()
    workflow_manager.execute_workflow('simple_backtest', config)
    ...
    if 'strategy_execution' in timing_data and timing_data['strategy_execution']:
        ...
        print(f"  Strategies per bar: {total_strategy_calls / max(1, len(timing_data['strategy_execution'])):.1f}")
    ...
    if 'feature_update_total' in timing_data and timing_data['feature_update_total']:
        ...
        feature_types = [k for k in timing_data.keys() if k.startswith('feature_') and not k.endswith('_total') and not k.endswith('_batch')]
        if feature_types:
    ...
    # Overhead analysis
    if 'total_execution' in timing_data and 'strategy_execution' in timing_data:
        total_exec_times = [t * 1000 for t in timing_data['total_execution']]
        strategy_exec_times = [t * 1000 for t in timing_data['strategy_execution']]
        readiness_times = [t * 1000 for t in timing_data['readiness_check']]
        
        print(f"\nOverhead Analysis (per bar):")
        print(f"  Total execution: {np.mean(total_exec_times):.2f}ms")
        print(f"  Strategy execution: {np.mean(strategy_exec_times):.2f}ms")
        print(f"  Readiness checks: {np.mean(readiness_times):.2f}ms")
        print(f"  Other overhead: {np.mean(total_exec_times) - np.mean(strategy_exec_times) - np.mean(readiness_times):.2f}ms")
