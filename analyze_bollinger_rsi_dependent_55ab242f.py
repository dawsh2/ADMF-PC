#!/usr/bin/env python3
"""
Analyze Bollinger RSI Dependent strategy performance in workspace 55ab242f
"""
...
# Load signals
signal_file = Path("/Users/daws/ADMF-PC/workspaces/signal_generation_55ab242f/traces/SPY_1m/signals/bollinger_rsi_dependent/SPY_compiled_strategy_0.parquet")
signals = pd.read_parquet(signal_file)

print("="*60)
print("BOLLINGER RSI DEPENDENT STRATEGY ANALYSIS")
print("Workspace: signal_generation_55ab242f")
print("="*60)
...
if len(trades_df) > 0:
    ...
    for i, trade in top_wins.iterrows():
        print(f"  {trade['pnl_pct']:.3f}% in {trade['duration']} bars ({trade['signal_type']})")
    
    print("\nLARGEST LOSING TRADES:")
    top_losses = trades_df.nsmallest(5, 'pnl_pct')
    for i, trade in top_losses.iterrows():
        print(f"  {trade['pnl_pct']:.3f}% in {trade['duration']} bars ({trade['signal_type']})")
...
# Check metadata
metadata_file = Path("/Users/daws/ADMF-PC/workspaces/signal_generation_55ab242f/metadata.json")
