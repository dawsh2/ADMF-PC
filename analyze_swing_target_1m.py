#!/usr/bin/env python3
"""Analyze Swing Pivot Bounce Target 1m optimization results."""
...
def extract_parameters(strategy_idx, param_space):
    ...
    # Parameter order from 1m config:
    # sr_period: [15, 20, 30, 40, 50, 60] = 6 values
    # min_touches: [3, 4, 5] = 3 values  
    # bounce_threshold: [0.003, 0.0035, 0.004, 0.0045] = 4 values
    # target_threshold: [0.0035, 0.0038, 0.004, 0.0042, 0.0045] = 5 values
    # Total: 6 * 3 * 4 * 5 = 360 combinations
    
    sr_periods = [15, 20, 30, 40, 50, 60]
    min_touches = [3, 4, 5]
    bounce_thresholds = [0.003, 0.0035, 0.004, 0.0045]
    target_thresholds = [0.0035, 0.0038, 0.004, 0.0042, 0.0045]
    
    # Calculate indices
    target_idx = strategy_idx % 5
    strategy_idx //= 5
...
def main():
    ...
    workspace_path = Path('/Users/daws/ADMF-PC/workspaces/signal_generation_374a8f29')
    
    print(f"Analyzing 1m optimization workspace: {workspace_path.name}")
    print("Strategy: swing_pivot_bounce_target on 1-minute data")
    ...
    if len(profitable) > 0:
        ...
        print("-" * 140)
        print(f"{'SR':<4} {'Touch':<6} {'Bounce':<8} {'Target':<8} {'Trades':<8} {'T/Day':<8} {'Win%':<8} {'Annual':<10} {'Avg(bps)':<10} {'Bars':<8}")
        print("-" * 140)
        ...
        for _, row in profitable.head(10).iterrows():
            print(f"{row['sr_period']:<4.0f} {row['min_touches']:<6.0f} {row['bounce_threshold']:<8.4f} "
                  ...
                  f"{row['win_rate']*100:<8.1f} {row['annual_return']*100:<10.2f} {row['avg_return_bps']:<10.1f} "
    ...
    # Compare with 15m results
    print("\n=== COMPARISON: 1m vs 15m ===")
    print("15m best result: -0.10% annual return, 1.03 trades/day")
    if len(results_df) > 0:
        best_1m = results_df.loc[results_df['annual_return'].idxmax()]
        print(f"1m best result: {best_1m['annual_return']*100:.2f}% annual return, {best_1m['trades_per_day']:.2f} trades/day")
        print(f"Trade frequency increase: {best_1m['trades_per_day']/1.03:.1f}x")
        print(f"Best 1m avg return: {best_1m['avg_return_bps']:.1f} bps per trade")
    
    # Bollinger comparison
    print("\n=== COMPARISON WITH BOLLINGER RSI ===")
    print("Bollinger RSI 15m: 60 bps/trade, 42 trades total")
    print("Bollinger RSI 5m: 11 bps/trade, 162 trades total")
    if len(profitable) > 0:
        best = profitable.iloc[0]
        print(f"Swing Bounce 1m best: {best['avg_return_bps']:.1f} bps/trade, {best['trades']:.0f} trades total")
    
    # Save results
    output_file = 'swing_target_1m_results.csv'
