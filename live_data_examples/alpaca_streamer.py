...
class AlpacaWebSocketStreamer:
    ...
    async def _authenticate(self) -> None:
        ...
        logger.debug(f"Parsed auth data: {auth_data}, type: {type(auth_data)}")
        ...
        elif auth_data.get("T") == "error" and auth_data.get("code") == 406:
            # Connection limit exceeded
            raise Exception(f"Connection limit exceeded - close other Alpaca WebSocket connections first")
        else:
            raise Exception(f"Authentication failed: {auth_data}")
    
    async def _subscribe(self) -> None:
        """Subscribe to bar data for configured symbols."""
        logger.debug(f"Sending subscribe payload: {self.subscribe_payload}")
        await self._websocket.send(json.dumps(self.subscribe_payload))
        
        # Wait for subscription response
        response = await self._websocket.recv()
        logger.debug(f"Received subscribe response: {response}")
    ...
    def _process_message(self, msg: Dict[str, Any]) -> Optional[StreamedBar]:
        """Process a single WebSocket message and return StreamedBar if it's bar data."""
        try:
            # Check if this is a bar message
            if msg.get("T") != "b":  # "b" indicates bar data
                return None
            ...
            # Log bar (for debugging)
            logger.debug(f"Received bar: {symbol} {timestamp} O:{bar.open} H:{bar.high} L:{bar.low} C:{bar.close} V:{bar.volume}")
